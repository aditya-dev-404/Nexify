...
function Profile() {
    const {user, posts, edit, setEdit} = useUser();
        ...
        <div className="info-holder max-w-[700px] mx-auto px-4 sm:px-0 flex flex-col gap-4">
            ...
            <div className="images relative">
                <div className="coverImage neo w-full h-[140px] sm:h-[220px] rounded-2xl overflow-hidden bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)]">
                    <img src={user.coverImage.url || cover} alt="cover image" className="w-full h-full object-cover" />
                </div>
                <div className="profileImage absolute left-6 sm:left-10 -bottom-10 sm:-bottom-14 h-20 w-20 sm:h-28 sm:w-28 rounded-full overflow-hidden border-4 border-[var(--surface)] shadow-[3px_3px_6px_var(--shadow-dark),-3px_-3px_6px_var(--shadow-light)]">
                    <img src={user.profileImage.url || dp} alt="profile image" className="w-full h-full object-cover" />
                </div>
            </div>

            {/* Name + basic info */}
            <div className="name-in-center neo rounded-2xl px-6 sm:px-10 pt-12 sm:pt-16 pb-6 flex flex-col gap-1">
                <h4 className="text-[var(--text)] text-lg sm:text-2xl font-semibold capitalize">{user.firstName + " " + user.lastName}</h4>
                <p onClick={()=> navigate('/networks')} className="text-[var(--text-muted)] capitalize !text-[12px] sm:text-base cursor-pointer">{user.connecions.length == 0 ? "no connections yet" : user.connecions.length == 1 ? "1 Connection" : user.connecions.length + " Connections"}</p>
                <p className="flex gap-2 items-center text-[var(--text-muted)] capitalize !text-[14px] sm:text-base">{user.location} <IoLocationSharp/></p>
                <p className="text-[var(--text-muted)] text-sm sm:text-base">{user.headings}</p>
<div className="w-full h-[1px] bg-[var(--border)] mt-2"></div>
            ...
            <div className="edit mt-5 flex justify-center">
            ...
            <div className="userPosts flex flex-col gap-4 mt-2">
                <h4 className='p-4 text-[var(--text-muted)]'>All Your Posts...</h4>
                {posts
                        ...
                        <div key={post._id} className="post-card neo p-4 sm:p-6 rounded-2xl w-full flex flex-col gap-4">
                            <div className="post-header flex gap-4">
                                <div className="image h-[30px] w-[30px] sm:h-[60px] sm:w-[60px] rounded-full overflow-hidden shrink-0 shadow-[3px_3px_6px_var(--shadow-dark),-3px_-3px_6px_var(--shadow-light)]">
                                    <img src={post.author.profileImage.url} alt="" className="h-full w-full object-cover" />
                                </div>
                            ...
                            <div className="like-comment-iconbox flex items-center justify-between gap-6 border-y border-[var(--border)] py-3 px-4 sm:px-8">
                                <div className="like flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--primary)] cursor-pointer transition text-sm sm:text-base">
                                    <AiFillLike size={20} /> <p>{post.likes.length}</p>
                                </div>
                                <div className="comment flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--primary)] cursor-pointer transition text-sm sm:text-base">
                                    <FaComments size={20} /> <p>{post.comments.length}</p>
                                </div>
                            </div>
                        </div>
                    ))
